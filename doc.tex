...
\begin{table*} \begin{tabularx}{\textwidth}{c|c|c|X}
	...
	\wire{burst\_reset} & 1 & I & Imediately resets the burst handler.
	\\
	...
	\wire{subchan\_data} & ? & I & See \autoref{tbl:frame-io}. This
...
\begin{table*} \begin{tabularx}{\textwidth}{c|c|c|X}
	...
	\wire{rand\_in\_bits}  & 1  & I & Uncoded input bitstream to the
	randomizer (this is the first step in the encoding process).\\

	\wire{rand\_in\_valid} & 1  & I & Indicates the input bitstream

	\wire{rand\_in\_bits} is valid and should be read. \\

	\wire{rand\_out\_bits} & 1  & O & Output bitstream. \\
	
	\wire{rand\_out\_bits} & 1  & O & Indicates the output bitstream
	is valid. \\

	\wire{rand\_iv}        & 15 & I & Initialization data for the
	internal register. \\

	\wire{rand\_reload}    & 1  & I & Indicates the internal
	register should be loaded with \wire{rand\_iv}. \\

	\wire{rand\_reset}     & 1  & I & Resets all internal state
...
\begin{table*} \begin{tabularx}{\textwidth}{c|c|c|X}
	...
	\wire{fec\_in\_bits} & 1 & I & Input bitstream, processsed by randomizer.
	\\

	\wire{fec\_in\_valid} & 1 & I & Indicates the input bitstream
	\wire{fec\_in\_bits} should be read.\\

	\wire{fec\_out\_bits} & 1 & O & Output bitstream, has FEC
	applied. Will be longer than input bitstream. \\

	\wire{fec\_out\_valid} & 1 & O & Indicates the output bitstream
...
\begin{table*} \begin{tabularx}{\textwidth}{c|c|c|X}
	...
	\wire{subch\_ct} & 3 & I & Indicates whether the number of subchannels
...
\begin{table*} \begin{tabularx}{\textwidth}{c|c|c|X}
	...
	\wire{FIXME SOME PARAM} & ? & I & Something needed to configure this
	device. \\
...
\begin{table*} \begin{tabularx}{\textwidth}{c|c|c|X}
	...
	\wire{in\_blk} & 200 * 2 * 16 & I & Input data for the IFFT. Width of
	numbers is 16 bits, processing both Q and I. \\

	\wire{out\_blk} & 256 * 2 * 16 & O & Output data after IFFT is preformed. Width of
	numbers is 16 bits, processing both Q and I. \\
	...
	\caption{IFFT input/output description.} \end{tabularx}
\end{table*}
